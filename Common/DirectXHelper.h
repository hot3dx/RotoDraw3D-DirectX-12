...
namespace DX
{
	...
	inline Concurrency::task<std::vector<mybyte>> ReadDataAsync(const std::wstring& filename)
	{
		...
		}).then([](Streams::IBuffer^ fileBuffer) -> std::vector<mybyte>
		{
			std::vector<mybyte> returnBuffer;
			returnBuffer.resize(fileBuffer->Length);
			Streams::DataReader::FromBuffer(fileBuffer)->ReadBytes(Platform::ArrayReference<mybyte>(returnBuffer.data(), fileBuffer->Length));
			return returnBuffer;
		});
	}
    ...
    {
        WCHAR fullName[50];
        StringCbPrintf(fullName, 50, L"%s[%u]", name, index);
        if (fullName > 0)
        {
...
inline HRESULT ReadDataFromFile(LPCWSTR filename, mybyte** data, UINT* size)
{
    ...
    *data = reinterpret_cast<mybyte*>(malloc(fileInfo.EndOfFile.LowPart));
