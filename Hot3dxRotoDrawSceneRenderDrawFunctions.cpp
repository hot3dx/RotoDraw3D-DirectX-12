...
void Hot3dxRotoDraw::RotoDrawSceneRender::DrawObjectPointsTop2()
{
	...
	if (m_bIsYAxis)
	{
		DirectX::DXTKXAML12::VertexPositionColor vpc = { XMFLOAT3(m_VectorBeginPoint.position.x * m_fScale1stLineDrawnPts, m_VectorBeginPoint.position.y, m_VectorBeginPoint.position.z * m_fScale1stLineDrawnPts), XMFLOAT4(r,g,b,alpha) };
		vertices.at(0) = vpc;
		k++;
	...
	{
		DirectX::DXTKXAML12::VertexPositionColor vpc = { XMFLOAT3(m_VectorBeginPoint.position.x, m_VectorBeginPoint.position.y * m_fScale1stLineDrawnPts, m_VectorBeginPoint.position.z * m_fScale1stLineDrawnPts), XMFLOAT4(r,g,b,alpha) };
		vertices.at(0) = vpc;
		k++;
...
void Hot3dxRotoDraw::RotoDrawSceneRender::DrawObjectPointsBottom2()
{
	...
	if (m_bIsYAxis)
	{
		DirectX::DXTKXAML12::VertexPositionColor vpc = { XMFLOAT3(m_VectorEndPoint.position.x * m_fScale1stLineDrawnPts, m_VectorEndPoint.position.y, m_VectorEndPoint.position.z * m_fScale1stLineDrawnPts), XMFLOAT4(r,g,b,alpha) };
		vertices.at(0) = vpc;
		k++;
	...
	{
		DirectX::DXTKXAML12::VertexPositionColor vpc = { XMFLOAT3(m_VectorEndPoint.position.x, m_VectorEndPoint.position.y * m_fScale1stLineDrawnPts, m_VectorEndPoint.position.z * m_fScale1stLineDrawnPts), XMFLOAT4(r,g,b,alpha) };
		vertices.at(0) = vpc;
		k++;
...
void Hot3dxRotoDraw::RotoDrawSceneRender::DrawObjectPointsTopBottom2()
{
	if (m_iPointCount <= 0) { return; }
		...
		size_t baseCount = m_iPointCnt + 2; // +2 for top and bottom endpoints
		size_t totalVerts = closed
			? (baseCount + baseCount * (cnt - 1) + 1)
			: (baseCount + baseCount * (cnt - 1));
		vertices.resize(totalVerts);
		ResizePtGroups(closed ? (cnt + 1) : cnt);
